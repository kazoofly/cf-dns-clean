...
sync_project_files() {
  ...
  mkdir -p "${app_dir}"
  cp -f "${SCRIPT_DIR}/docker-compose.image.yml" "${app_dir}/"
}
...
ensure_key_file() {
  ...
  printf '请输入 Docker 镜像部署使用的加密主密钥（可留空，留空则自动生成）: '
  read -r -s master_key
  printf '\n'
...
main() {
  ...
  local image_name="${1:-${CF_DNS_BOT_IMAGE:-}}"

  require_root "$@"
  ensure_docker

  [ -n "${image_name}" ] || fail "请传入预构建镜像名，例如：bash docker-image-deploy.sh ghcr.io/your/repo:latest"

  state_dir="${2:-${STATE_DIR_DEFAULT}}"
  app_dir="${state_dir}/app"
  data_dir="${state_dir}/data"
  env_file="${state_dir}/docker.env"
  key_file="${state_dir}/secrets/master.key"
  compose_file="${app_dir}/docker-compose.image.yml"
  ...
  if ! docker compose --env-file "${env_file}" -f "${compose_file}" pull; then
    ...
    fail "Docker 镜像拉取失败，已从 docker.env 移除明文 Telegram 启动信息。"
  fi
